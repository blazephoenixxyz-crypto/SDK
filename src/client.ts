...
export class BlazePhoenix {
  ...
  quote(req: QuoteRequest): Promise<QuoteResponse> {
    return this.call<QuoteResponse>(`/api/quote?${toSearch(req).toString()}`);
  }
  ...
  quoteBatch(reqs: QuoteRequest[]): Promise<BatchResponse> {
    return this.call<BatchResponse>('/api/quote/batch', {
      method: 'POST',
  ...
  manifest(): Promise<ManifestResponse> {
    return this.call<ManifestResponse>('/api/manifest');
  }
}
